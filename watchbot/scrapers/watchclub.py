...
class WatchClubScraper(BaseScraper):
    name = "watchclub"
    _BASE = "https://www.watchclub.com/search?q={query}"
    ...
    def _parse_listings(self, soup, target: TargetWatch) -> list[Listing]:
        listings = []
        for card in soup.select("[class*='product'], article, li[class*='item'], [class*='watch']"):
            try:
                ...
                if not url.startswith("http"):
                    url = "https://www.watchclub.com" + url
                ...
                logger.debug("watchclub parse error: %s", e)
        return listings

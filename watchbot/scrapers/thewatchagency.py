...
class TheWatchAgencyScraper(BaseScraper):
    name = "thewatchagency"
    _BASE = "https://www.thewatchagency.com/search?q={query}"
    ...
    def _parse_listings(self, soup, target: TargetWatch) -> list[Listing]:
        listings = []
        cards = soup.select("[class*='product'], [class*='watch'], article, li[class*='item'], div[class*='listing']")
        for card in cards:
    ...
    def _parse_card(self, card, target: TargetWatch) -> Listing | None:
        try:
            ...
            if not url.startswith("http"):
                url = "https://www.thewatchagency.com" + url
            ...
            logger.debug("thewatchagency parse error: %s", e)
            return None

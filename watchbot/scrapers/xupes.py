...
class XupesScraper(BaseScraper):
    name = "xupes"
    _BASE = "https://xupes.com/search?q={query}"
    ...
    def _parse_listings(self, soup, target: TargetWatch) -> list[Listing]:
        listings = []
        cards = soup.select("[class*='product'], [class*='watch'], article, li[class*='item']")
        for card in cards:
    ...
    def _parse_card(self, card, target: TargetWatch) -> Listing | None:
        try:
            ...
            return Listing(source=self.name, listing_id=lid, url=url, title=title,
                           price_gbp=price, is_exact_ref_match=True,
                           has_full_set=self._detect_full_set(full_text))
        except Exception as e:
            logger.debug("xupes parse error: %s", e)
            return None

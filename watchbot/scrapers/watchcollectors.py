...
class WatchCollectorsScraper(BaseScraper):
    name = "watchcollectors"
    _BASE = "https://watchcollectors.co.uk/search?q={query}&type=product"
        ...
        for card in soup.select("[class*='product'], article, li[class*='item']"):
            try:
                ...
                if not url.startswith("http"):
                    url = "https://watchcollectors.co.uk" + url
                ...
                logger.debug("watchcollectors parse error: %s", e)
        return listings

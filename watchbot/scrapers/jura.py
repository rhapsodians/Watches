...
class JuraScraper(BaseScraper):
    name = "jura"
    _BASE = "https://www.jurawatches.co.uk/search?q={query}&type=product"
        ...
        for card in soup.select("[class*='product'], article, li[class*='item']"):
            try:
                ...
                if not url.startswith("http"):
                    url = "https://www.jurawatches.co.uk" + url
                ...
                logger.debug("jura parse error: %s", e)
        return listings

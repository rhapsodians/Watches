...
class WatchTraderScraper(BaseScraper):
    name = "watchtrader"
    _BASE = "https://www.watchtrader.co.uk/?search_term={query}"

    def search(self, target: TargetWatch) -> list[Listing]:
        query = self._url_encode(target.reference)
        soup = self._fetch(self._BASE.format(query=query), wait_selector="[class*='listing'], [class*='watch'], article")
        if soup is None:
            return []
    ...
    def _parse_listings(self, soup, target: TargetWatch) -> list[Listing]:
        listings = []
        cards = soup.select("[class*='listing'], [class*='watch-item'], [class*='product'], article, li[class*='item']")
        for card in cards:
    ...
    def _parse_card(self, card, target: TargetWatch) -> Listing | None:
        try:
            ...
            if not url.startswith("http"):
                url = "https://www.watchtrader.co.uk" + url
            ...
            logger.debug("watchtrader parse error: %s", e)
            return None
